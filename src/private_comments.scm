...
### Notes:
See `comments-for-file` documentation for info on the file structure.
")
...
(doc-fun "comments-for-file"

"## Private: comments-for-file [project-hash file-path-hash treeishes]

### Parameters:
* project-hash   - String (see notes)
* file-path-hash - String (see notes)
* treeishes      - List of strings indicating the treeishes we care
                   about for the specified file

### Returns:
A list of alists, or '(). Each alist contains comment data that the
user has associated with one of the specified treeishes for the
specified file path.

Most of the time this will return an empty list, because most
files don't have private comments associated with them.
...
```
")
(define (comments-for-file project-hash file-path-hash treeishes )
        ...
        (let* ((treeish-dir (list->path (list project-dir treeish)))
          ...
          (do-list file files
           ...
           (let ((hash-from-filename (filename->path-hash file))
                 ; hash-from-filename is either a 65 char hash OR "NOT-A-COMMENT-FILE"
                 ; it *should* never be NOT-A-COMMENT-FILE
